...
CREATE TABLE words(
   id_words INT DEFAULT AUTO_INCREMENT,
...
CREATE TABLE deliver_to(
   ...
   PRIMARY KEY(id_subscription, id_shipping_type),
...
CREATE TABLE contains(
   ...
   quantity INT,
   PRIMARY KEY(id_equipment, id_shopping_cart),
   FOREIGN KEY(id_equipment) REFERENCES equipment(id_equipment),
...
CREATE TABLE is_open_at(
   id_opening_hours INT,
   id_relay_point INT,
   PRIMARY KEY(id_opening_hours, id_relay_point),
   FOREIGN KEY(id_opening_hours) REFERENCES opening_hours(id_opening_hours),
   FOREIGN KEY(id_relay_point) REFERENCES relay_point(id_relay_point)
);
...
CREATE TABLE censure(
   id_users INT,
   id_words INT,
   PRIMARY KEY(id_users, id_words),
   FOREIGN KEY(id_users) REFERENCES users(id_users),
   FOREIGN KEY(id_words) REFERENCES words(id_words)
);
