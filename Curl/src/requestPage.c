...
void handle_request(GtkButton *button, gpointer content)
{
    ...
    if (curl)
    {
        ...
        {
            char postFields[256]; // Définissez la taille appropriée pour les données POST

            // Construisez les données POST au format clé=valeur
            //snprintf(postFields, sizeof(postFields), "param1=%s&param2=%s", value1, value2);

            // Définissez les options de requête POST
